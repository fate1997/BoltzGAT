...
class GATv2ConcatModel(nn.Module):
    def __init__(self, config):
        ...
        self.pred_head = MLP(2*self.hidden_dim + 1, 256, 1, 3, 0.2, nn.ELU())
    ...
    def forward(self, data):
        ...
        temp_repr = torch.cat([mol_repr, data.temps], dim=-1).to(x.device)
        if self.return_repr:
            return self.pred_head(temp_repr), mol_repr
        return self.pred_head(temp_repr)